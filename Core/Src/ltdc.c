...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef* ltdcHandle)
{
  ...
  /* USER CODE END LTDC_MspInit 0 */
    ...
    */
    ...
    /* LTDC interrupt Init */
    HAL_NVIC_SetPriority(LTDC_IRQn, 0, 10);
...
void LCD_Fill(uint16_t x,uint16_t y,uint16_t width,uint16_t high,uint32_t color){
		uint8_t *start_adr = (uint8_t *)(SDRAM_BANK1_START_ADRESS + x*2 + y*800*2) ;
	  uint32_t total = width * high * 2;
		uint32_t j;
		for(j=0;j<total;j++){
			*start_adr = color;
			start_adr++;
...
void LCD_Draw_Rectangle(uint16_t x0,uint16_t y0,uint16_t x1,uint16_t y1,uint16_t *address){
		uint16_t *CpyAdr = address;
		uint16_t *OffsetAdr;
	  volatile uint16_t j=0,k=0;
//	 OffsetAdr = (uint16_t *)(SDRAM_BANK1_START_ADRESS + x0*2 + j*800*2);
		for(j=y0;j<=y1;j++)
	  {
			 OffsetAdr = (uint16_t *)(SDRAM_BANK1_START_ADRESS + x0*2 + j*800*2);
			 for(k=x0;k<=x1;k++)
...
//void LCD_Draw_Picture(uint16_t x,uint16_t y,uint16_t width,uint16_t high,uint8_t *address){
//		uint32_t *CpyAdr = address;
//		uint32_t *start_adr = (uint32_t *)(SDRAM_BANK1_START_ADRESS + x*3 + y*800*3) ;
//		uint32_t temp0 = (high * width *3)%4;         //不可四字节对齐的字节数量;
//		uint32_t temp1 = (high * width *3) - temp0;   //可四字节对齐的字节数量;
//		uint32_t j;
//		for(j=0;j<temp1;j++){
//			*start_adr = *CpyAdr;
//			start_adr++;
//			CpyAdr++;
//		}
//		start_adr = (uint8_t *)start_adr;
//		CpyAdr    = (uint8_t *)CpyAdr;
//		for(j=0;j<temp0;j++){
//			*start_adr = *CpyAdr;
//			start_adr++;
//			CpyAdr++;
