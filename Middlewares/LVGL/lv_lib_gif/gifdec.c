...
read_image_data(gd_GIF *gif, int interlace)
{
    ...
    while (frm_off < frm_size) {
        ...
        //entry = table->entries[key];
				entry.length = table->entries[key].length;
				entry.prefix = table->entries[key].prefix;
				entry.suffix = table->entries[key].suffix;
        ...
        for (i = 0; i < str_len; i++) {
            ...
            if (entry.prefix == 0xFFF)
                ...
                //entry = table->entries[entry.prefix];
								entry.length = table->entries[entry.prefix].length;
								entry.prefix = table->entries[entry.prefix].prefix;
								entry.suffix = table->entries[entry.prefix].suffix;
						}
        }
        frm_off += str_len;
